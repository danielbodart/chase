...
let
  ...
  # ./github/exceptions.json saying where the method is wrong, and adding
  # GraphQL -- one POST for every query and mutation, so a write until frisket
  # can tell them apart. See ../docs/github.md.
    ...
    type = types.attrsOf (types.submodule ({ config, ... }: {
      options.apps.github = ops.appOptions config // {
        enable = mkEnableOption ''
          ...
          a read goes straight through, and the rest -- every GraphQL request
          among them -- is answered as `writes`, `guarded` and `unmatched`
          say'';
