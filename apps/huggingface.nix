...
let
  ...
  # Cloudflare's are: admitted if it is known to be harmless, asked about
  # otherwise. Generated by ../scripts/operations.sh from the spec vendored in
  # ./huggingface -- the Hub publishes it at a URL that moves, not at a
  # commit -- with ./huggingface/exceptions.json saying which reads are not
  # harmless (the ones that mint a token), which writes are, and what the
  # spec leaves out that huggingface_hub needs. See ../docs/huggingface.md.
    ...
    type = types.attrsOf (types.submodule {
      options.apps.huggingface = {
        enable = mkEnableOption ''
          ...
          with your token, and everything else -- an upload, a new
          repository, a deletion, a read that mints a token, and anything the
          description does not name -- waits for a person to allow it,
          through frisket's asker'';
  ...
  config = {
    ...
    services.frisket.policies = lib.mapAttrs (name: tier: mkIf tier.apps.huggingface.enable {
      ...
      routes.huggingface = {
        ...
        upstream = "https://${host}";
      } // (if tier.apps.huggingface.anonymous then {
        # Intercepted with no credential, so the scope holds: a token the
        # session brings still reaches the Hub, but only for what is known
        # to be harmless. What would ask is refused -- the reads that mint a
        # token, Xet's write token among them, with which a session could put
        # bytes in Xet's store with no request here at all -- and so is
        # anything the description does not name.
        paths = map (r: removeAttrs r [ "ask" ] // lib.optionalAttrs r.ask { refuse = true; }) operations;
      } else {
        # Downloads, gated models and Xet's read token go straight through.
        # Writes ask, as do the reads that mint a token, and anything nobody
        # has classified.
